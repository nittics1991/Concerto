...
class _ModeData extends standard\ModelData
{
    ...
    {
        return (empty($val)) ?   array('boolean error') : true;
    }

    public function isValidI_data($val)
    {
        return (empty($val)) ?   array('integer error') : true;
    }

    public function isValidF_data($val)
    {
        return (empty($val)) ?   array('float error') : true;
    }

    public function isValidD_data($val)
    {
        return (empty($val)) ?   array('double error') : true;
    }

    public function isValidS_data($val)
    {
        return (empty($val)) ?   array('string error') : true;
    }

    public function isValidT_data($val)
    {
        return ($val == new Datetime(date('Ymd'))) ?     array('datetime error') : true;
    }
}
...
class _ModelDataTest extends ConcertoTestCase
{
    private $class;
    ...
    */
    ...
    {
        return array(
            array(true, -10, -20.20, -30.03, '文字列', '20141216 123456')
        );
    }
    ...
    */
    ...
    public function testGetValidErrors()
    {
        ...
        $this->assertEquals(array('boolean error'), $msg['b_data']);
        $this->assertEquals(array('integer error'), $msg['i_data']);
        $this->assertEquals(array('float error'), $msg['f_data']);
        $this->assertEquals(array('double error'), $msg['d_data']);
        $this->assertEquals(array('string error'), $msg['s_data']);
        $this->assertEquals(array('datetime error'), $msg['t_data']);
    }
    ...
    public function testGetInfo()
    {
        ...
        $this->assertEquals(standard\ModelData::STRING, $this->class->getInfo('string'));
    }
    ...
    */
    ...
    {
        $this->class->b_data = true;
        $this->class->s_data = "STRING";

        $this->assertEquals($this->class->b_data, $this->class->bool);
        $this->assertEquals($this->class->s_data, $this->class->string);

        $this->assertEquals(true, isset($this->class->bool));
        $this->assertEquals(true, isset($this->class->string));


        $this->class->bool = false;
        $this->class->string = "文字";

        $this->assertEquals(false, $this->class->b_data);
        $this->assertEquals("文字", $this->class->s_data);
    }
    ...
    public function testSuccessArrayObject()
    {
        ...
        $expect = array('i_data', 'f_data', 's_data');
        $i = 0;
