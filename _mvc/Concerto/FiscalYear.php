...
class FiscalYear
{
    ...
    */
    public static function codeToDatetime(
        ...
        $half_start = DateTimeImmutable::createFromFormat(
            '!Yn',
            $parsed_code['year'] .
                (string)static::HALF_START_MONTH
        );
        ...
        $half_start2 = $half_start->modify('first day of today');

        /** @var \DateTimeImmutable|false $half_start2 */
        if (!$half_start2) {
            return null;
        }

        return $parsed_code['code'] === static::HALF_CODES[0] ?
            $half_start2 :
            $half_start2->add(
                new DateInterval("P6M")
            );
    ...
    *   @return string[][] [['kb_nendo' => '', 'nm_nendo' => ''], ...]
    */
    public static function getNendoPeriodCollection(
        ...
        do {
            ...
            $items['nm_nendo'] = strval(
                static::nendoCodeToZn($current)
            );
