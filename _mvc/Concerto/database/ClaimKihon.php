...
class ClaimKihon extends ModelDb
{
    ...
    *   @inheritDoc
    */
    protected string $schema = 'public.claim_kihon';
    ...
    *   @return int
    */
    public function lastFiscalMonthCount(
        string $kb_nendo,
        string $cd_bumon
    ): int {
        $sql = "
    ...
    *   @return mixed[]
    */
    public function occurenceClaimSummary(
        string $kb_nendo,
        string $cd_bumon
    ): array {
        $sql = "
            SELECT SUBSTR(dt_hassei, 1, 4) ||
                SUBSTR(dt_hassei, 6, 2) AS dt_hassei
            ...
            GROUP BY SUBSTR(dt_hassei, 1, 4) ||
                SUBSTR(dt_hassei, 6, 2)
            ORDER BY dt_hassei
        ";
    ...
    */
    public function processedClaimSummary(
        ...
        $sql = "
            SELECT SUBSTR(dt_kakunin, 1, 4) ||
                SUBSTR(dt_kakunin, 6, 2) AS dt_kakunin
            ...
            GROUP BY SUBSTR(dt_kakunin, 1, 4) ||
                SUBSTR(dt_kakunin, 6, 2)
            ORDER BY dt_kakunin
        ";
    ...
    *   @return string
    */
    protected function formatStartDate(
        string $dt_yyyymm
    ): string {
    ...
    *   @return string
    */
    protected function formatEndDate(
        string $dt_yyyymm
    ): string {
        return mb_substr($dt_yyyymm, 0, 4) . '/' .
