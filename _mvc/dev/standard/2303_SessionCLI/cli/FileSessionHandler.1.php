 ...
 *
 * This script is based on php example on how to implement SessionHandlerInterfa
ce see http://php.net/manual/en/class.sessionhandlerinterface.php
 * with a notable difference :
 * - we do not write data back to file when data did not change
 *
 * Objectives :
 * ------------
 * When loading images (and other static content) the browser may decide to use
multiple TCP connections (concurrent) to get the data as fast as possible.
 * This also happens when using AJAX.
 * This may (and will most likely) lead to different workers (threads) on the we
b server answering these concurrent requests concurrently.
 *
 * The objective of this script is to improve speed for those cases of concurren
t requests.
 * The drawback is race conditions may happen if you need to write data to sessi
on.
 *
 *
 * References :
 ...
 * - about session locking : https://www.leaseweblabs.com/2013/10/symfony2-memca
che-session-locking/
 * - about race conditions : http://thwartedefforts.org/2006/11/11/race-conditio
ns-with-ajax-and-php-sessions/
 *
 *
 * How to use :
 * -------------
