...
DECLARE
  _version INTEGER;
BEGIN
  SELECT replace(setting, ''.'', '''')::integer INTO _version
    FROM pg_settings
...
DECLARE
  ...
  _r RECORD;
BEGIN
  FOR _r IN SELECT pg_current_wal_lsn(),
                   pg_current_wal_insert_lsn() LOOP

    INSERT INTO pgperf.snapshot_pg_current_xlog (sid,
                                                 location,
                                                 insert_location)
                                          VALUES (_sid,
                                                  _r.pg_current_wal_lsn,
                                                  _r.pg_current_wal_insert_lsn);
  END LOOP;
